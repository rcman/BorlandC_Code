...
void checkshots(void)
{
		...
		if (distance > 60 || first==0)
		{

		totalshots[shottotal].x=shipx;
		//totalshots[shottotal].y=spy;
		totalshots[shottotal].flag=1;
...
void moveshots(void)
{
	...
	for (i=0;i<shottotal;i++)
	{
			...
			tempx = totalshots[i].x;
			tempy = totalshots[i].y;
		...
		distance = distance + 1;
		tempy = tempy -1;
		totalshots[i].y=tempy;

		if (totalshots[i].y < 1 && totalshots[i].flag==0) 	// its gone off screen
			{
			totalshots[i].y=1;
			totalshots[i].flag=1;
			shottotal = shottotal - 1;
