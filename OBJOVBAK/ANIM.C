...
static char screens[12][20] ={
	{ 0x02, 0x00, 0x00, 0x00, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,0x02, 0x03, 0x02, 0x02,0x02, 0x02, 0x02, 0x02 },
...
void initarray(void)
{
	...
	for (c=0 ; c<numspr; c++)
	{
		...
		shipanim[c].shipspeed=random(2)+1;
...
void Animate(void)
{
	...
	if (s >= send)
		...
		for (i=0;i<16;i++)
		{
			...
			for (j=0;j<16;j++)
			{
				*(destin+j+vz+yval)=fshp[s].shp[i*16+j];
			}
			vz = vz + 320;
...
void PutImage(int spx,int spy,int spritenum)
{
	...
	for (x=0;x<8;x++)
	{
		...
		{
//		 *(destin+y+offset)=fshp[48].shp[x*16+y];
		...
		for (s=0;s<20;s++)
		{
			if (screens[t][s]==22)
			{
				for (x=0;x<16;x++)
				{
					for (y=0;y<16;y++)
					{
					*(destin+y+offset+ap+dn)=fshp[22].shp[x*16+y];;
					}
					offset = offset + 320;      //bytes to next line
				}
				offset = 0;
			...
			if (screens[t][s]==2)
			{
				for (x=0;x<16;x++)
				{
					for (y=0;y<16;y++)
					{
				*(destin+y+offset+ap+dn)=fshp[31].shp[x*16+y];
			...
			if (screens[t][s]==3)
			 {
				 for (x=0;x<16;x++)
				{
					 for (y=0;y<16;y++)
					{
					*(destin+y+offset+ap+dn)=fshp[46].shp[x*16+y];
					}
					 offset = offset + 320;      //bytes to next line
				 }
				 offset = 0;
