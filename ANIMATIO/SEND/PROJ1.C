...
//	
  ...
  //	fshp[1].y=spy;
	...
	while (ext != 256)
	{
		...
		//			gotoxy(24,10);
		 //			printf("%d",layarr[sty][stx]);

					if(layarr[sty+1][stx]==51)
					{
					tempx = tempx + animobjects[0].animspeed;
					animobjects[0].animx=tempx;
					}

					if(layarr[sty][stx+1]==51)
					{
			...
			/*
					CLocX -=1;		// Check the Place of the Map

						if (CLocX >1)
						{
						copypageLtoR(dest+1,dest+0);
					  //	copycentre((dest+0x2820), (screen+0x2820),64,154,64);
						}
								if (CLocX <1)
								{
			  ...
			  //	Animate(6,animobjects[1].currentshape, animobjects[1].animx,animobjects[1].animy,20);
				  ...
				  /*			if (animobjects[1].currentshape==animobjects[1].max)
					...
					CLocX +=1;
							...
							if (CLocX <=48)		// temporary Max is 100
							{
							 EraseSingleLine();
