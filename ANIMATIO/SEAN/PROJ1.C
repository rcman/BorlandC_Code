			 ...
			 tblkx +=1;
				...
				if (tblkx ==20)
					...
					tblky +=1;
						...
						{
								gotoxy(5,25);
								printf("%d  %d %d %d",tblkx,tblky,layarr[tblkx][tblky],fshp[1].x=spx);
								above = layarr[tblky][tblkx];
								rght	= layarr[tblky][tblkx+1];
								lft = layarr[tblky][tblkx-1];
...
void PutImage(int spx,int spy, int spritenum)
{
	...
	for (x=0;x<16;x++)
	{
		for (y=0;y<16;y++)
		{
		  *(screen+y+offset)^=seqfshp[spritenum].shp[x*16+y];;
		}
		offset = offset + 320;      //bytes to next line
	}
