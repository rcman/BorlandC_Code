  ...
  //	PutImage(spx,spy,60);
	...
	while (ext != 256)
	{
		...
		if (bioskey(1))
		{
			...
			{
				 //	Check_Location(fshp[1].x,fshp[1].y);
				if (allowu==1)
				{
					spy = fshp[1].y;
					spy = spy -1;
...
int fileread(void)
{
	...
	if ((in = fopen("sample5.std", "rb")) == NULL)
	{
...
int fileread2(void)
{
  ...
  //	ClearData();		// Clear the Array
	...
	if ((mapin = fopen(smap, "rb")) == NULL)
	{
		fprintf(stderr, "Cannot open Standard file.\n");
		return 1;
	}

	if ((in = fopen(infst, "rb")) == NULL)
	{
		fprintf(stderr, "Cannot open input file.\n");
		return 1;
	}
				...
				if (tblkx ==20)
					...
					tblky +=1;
						...
						if (dis <16 && dis > 0)
						{
								...
								gotoxy(5,25);
								printf("%d  %d %d %d %d %d",tblkx,tblky,layarr[tblkx][tblky],fshp[1].x=spx,rght,lft);
